...
const Topbar = () => {
  ...
  const sidebarRoutes = [
    { label: "Курси", path: "/instructor/courses" },
    {
      label: "Статистика",
  ...
  return (
    <div className="flex justify-between items-center p-4">
      ...
      <div className="max-md:hidden w-[400px] rounded-full flex">
        ...
        />
        <button
          className="bg-[#1E5B4F] rounded-r-full border-none outline-none cursor-pointer px-4 py-3 hover:bg-[#2E7366]/80"
          disabled={searchInput.trim() === ""}
          onClick={handleSearch}
        >
          <Search className="h-4 w-4 text-[#FFF8EB]" />
        </button>
      </div>
              ...
              <div className="flex flex-col gap-4">
                {topRoutes.map((route) => (
                  <Link
                    href={route.path}
                    key={route.path}
                    className="text-sm font-medium hover:text-[#1E5B4F]"
                  >
                    {route.label}
                  </Link>
                ))}
